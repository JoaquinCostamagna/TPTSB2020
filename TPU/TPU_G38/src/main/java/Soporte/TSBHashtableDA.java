...
public class TSBHashtableDA<K,V> implements Map<K,V>, Cloneable, Serializable
{
    ...
    public V get(Object key) 
    {
        ...
        int ik = h((K)key);
        Map.Entry<K,V> entry = search_for_entry((K)key, ik);
    ...
    public V remove(Object key)
    {
        ...
        if (pos != -1) // Si la clave está en la tabla.
        {
            old = ((Entry<K,V>)table[pos]).getValue();
            Map.Entry<K,V> borrado = new Entry<>(null, null, TOMBSTONE);
            table[pos] = borrado;
            this.count--;
            this.modCount++;
        }
    ...
    private class KeySet extends AbstractSet<K> 
    {
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            // índice del elemento anterior (requerido para remove()).
            private int previous_entry;

            // índice del elemento actual en el iterador (el que fue retornado por next la última vez, y será eliminado
            // por remove().
            ...
            {
                previous_entry = current_entry = -1;
                next_ok = false;
            ...
            {
                // Para evitar realizar el casting a Entry en cada vuelta del ciclo for.
                Entry<K,V>[] t = (Entry<K, V>[]) TSBHashtableDA.this.table;

                if (TSBHashtableDA.this.isEmpty()) return false; //Si la lista está vacía.
                if (current_entry >= t.length) return false;// Si estoy parado al final de la tabla.

                for (int next_pos = current_entry + 1; next_pos < table.length; next_pos++){
                    if (t[next_pos].getState() == CLOSED) // Si la Entry no está vacía.
                    {
                        return true;
                    }
                }
            ...
            public K next() 
            {
                ...
                if(!hasNext())
                {
                    throw new NoSuchElementException("next(): no quedan elementos en la tabla.");
                }

                Entry<K,V>[] t = (Entry<K, V>[]) TSBHashtableDA.this.table;

                // Guardo la referencia del Entry anterior, que sería sobre el cual estoy posicionado.
                previous_entry = current_entry;
                // Busca el siguiente Entry no vacío, que debe existir, ya que hasNext() devolvió true.
                do {
                    current_entry++;
                }
                while(t[current_entry].getState() != CLOSED);
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> 
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            K key = entry.getKey();
            int ik = TSBHashtableDA.this.h(key);
            if (TSBHashtableDA.this.search_for_entry(key,ik).equals(entry)){
                return true;
            }
            else
                return false;
        ...
        public boolean remove(Object o) 
        {
            ...
            K key = entry.getKey();
            int ik = TSBHashtableDA.this.h(key);
            int pos = TSBHashtableDA.this.search_for_index(key,ik);
            if (pos != -1) // Si se encontró un Entry con la clave indicada.
                if (table[pos].equals(entry)) {
                    Map.Entry<K, V> borrado = new Entry<>(null, null, TOMBSTONE);
                    table[pos] = borrado;
                    return true;
                }
    ...
    private class ValueCollection extends AbstractCollection<V> 
    {
        ...
        private class ValueCollectionIterator implements Iterator<V>
        {
            ...
            /*
             * Crea un iterador comenzando en el primer entry. Activa el
             * mecanismo fail-fast.
             */
